...
def add_to_cohorts(customer: Customer, cohorts: List[Tuple[dt.datetime, int, List[int]]]) -> None:
    #Figure out which cohort
    cohort = (cohorts[0][0] - customer.created) // dt.timedelta(7)
    #Figure out how to modify cohorts[cohort][2]
    ...
    if (len(cohorts[cohort][2]) < max_purchase_week + 1):
        cohorts[cohort][2].extend([0]*(max_purchase_week + 1 - len(cohorts[cohort][2])))
